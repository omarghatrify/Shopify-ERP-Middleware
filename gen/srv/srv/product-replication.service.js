...
const ProductReplicationService_1 = require("#cds-models/ProductReplicationService");
...
async function replicateProductToShopify(client, row) {
    ...
    try {
        ...
        const edges = variantData?.productVariants?.edges ?? [];
        const now = new Date().toISOString();
        let shopify_product_id;
        let shopify_variant_id;
        let shopify_inventory_id;
        if (edges.length === 0) {
            const name_en = row.name?.find((n) => n.lang?.toUpperCase() === 'EN')?.val ||
                row.name?.[0]?.val ||
                'Default';
            const desc_en = row.desc?.find((d) => d.lang?.toUpperCase() === 'EN')?.val ||
                row.desc?.[0]?.val ||
                'Default';
            const productInput = {
                title: name_en,
            ...
            const productSet = createResult?.productSet;
...
exports.default = cds_1.default.service.impl(async function (srv) {
    ...
    srv.before(ProductReplicationService_1.SyncProducts, async (req) => {
        const { correlationId, materials, overwrite } = req.data;
        if (!correlationId) {
            req.data.correlationId = cds_1.default.utils.uuid();
        }
    ...
    srv.before(ProductReplicationService_1.LoadProducts, async (req) => {
        const { value } = req.data;
        if (!value || !Array.isArray(value) || value.length === 0) {
            req.error(400, 'value array is required');
        }
    });
    srv.on(ProductReplicationService_1.LoadProducts, async function (req) {
        ...
        const tx = srv.tx(req);
        return tx.send({
            event: ProductReplicationService_1.SyncProducts,
            data: {
                correlationId: correlationId ?? cds_1.default.utils.uuid(),
                materials,
                overwrite: typeof overwrite === 'boolean' ? overwrite : true,
            },
        });
    });
    srv.on(ProductReplicationService_1.SyncProducts, async (req) => {
        ...
        const statuses = await Promise.all(materials.map(async (mat) => {
            const product = await tx.run(SELECT.one.from(ProductReplicationService_1.Products, mat.sku));
            if (product) {
                console.log('SyncProducts on', mat.sku, 'EXISTING');
                await tx.run(UPDATE.entity(ProductReplicationService_1.Products, mat.sku).set({
            ...
            await tx.run(INSERT.into(ProductReplicationService_1.Products).entries({
                sap_material: mat.sku,
                uom: mat.uom,
                name: mat.name,
                desc: mat.desc,
    ...
    cds_1.default.on('served', async () => {
        ...
        cds_1.default.spawn({
            ...
            try {
                ...
                const products = (await tx.run(SELECT.from(ProductReplicationService_1.Products).where({ shopify_inventory_id: null }).limit(120)));
                console.log(`Found ${products.length} products to replicate`);
                const payloads = await Promise.all(products.map((row) => replicateProductToShopify(client, row)));
                const toPersist = payloads.filter((p) => p != null);
                await Promise.all(toPersist.map((p) => tx.run(UPDATE.entity(ProductReplicationService_1.Products, p.sku).set({
...
//# sourceMappingURL=product-replication.service.js.map